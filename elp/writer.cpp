...
ElpWriter::ElpWriter(const string &filename) : filename(filename) {
    fp = fopen(filename.c_str(), "wb");
    if (fp == null) throw FileNotFoundError(filename);
}
...
void ElpWriter::write(__Lcon con) {
    write(con.len);
...
void ElpWriter::write(MethodInfo info) {
    ...
    write(info.lineNumberTableCount);
    for (int i = 0; i < info.lineNumberTableCount; ++i) {
        auto line = info.lineNumberTable[i];
        write(line.byteCode);
        write(line.sourceCode);
    }
    write(info.meta);
}
...
void ElpWriter::write(ClassInfo info) {
    write(info.constantPoolCount);
    for (int i = 0; i < info.constantPoolCount; ++i) {
        write(info.constantPool[i]);
    }
    write(info.type);
    write(info.accessFlags);
