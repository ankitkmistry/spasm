...
void AssemblyBaseVisitor::State::addLabel(const string &label) {
    if (labels.find(label) != labels.end()) throw AssemblerError("duplicate label", label);
    labels[label] = pc;
}
...
uint16 AssemblyBaseVisitor::State::getJump(const string &label) {
    ...
    if (locItr != labels.end()) {
        ...
        if (unresolvedLabels.find(label) == unresolvedLabels.end()) unresolvedLabels[label] = {pc};
        else
...
void AssemblyBaseVisitor::State::resolveLabels() {
    ...
    for (auto [label, locations]: unresolvedLabels) {
        ...
        if (labelLocItr == labels.end()) throw AssemblerError("cannot find label", string(label));
        ...
        for (auto location: locations) {
            ...
            switch (paramCount) {
                ...
                case 1:
                    ...
                    break;
                default:
                    throw AssemblerError(format("opcode expects %d parameters", paramCount), OpcodeInfo::toString(opcode));
            }
        }
        // Make it resolved
        resolvedLabels.push_back(label);
    }
    for (auto label: resolvedLabels) { unresolvedLabels.erase(label); }
    // Complain if some labels are still not resolved
    vector<string> unresolved;
    unresolved.reserve(unresolvedLabels.size());
    for (auto [label, _]: unresolvedLabels) unresolved.push_back(label);
    if (!unresolvedLabels.empty()) throw AssemblerError("cannot find labels", listToString(unresolved));
}
...
MethodInfo::LineInfo AssemblyBaseVisitor::State::getLineTable() {
    ...
    for (int i = 0; i < lineInfo.numberCount; ++i) {
        lineInfo.numbers[i] = {.times = lines[i].times, .lineno = lines[i].lineno};
    }
    return lineInfo;
}
...
std::any AssemblyBaseVisitor::visitAssembly(AssemblyParser::AssemblyContext *ctx) {
    ...
    for (size_t i = 0; i < elp.globalsCount; ++i) { elp.globals[i] = any_cast<GlobalInfo>(visitGlobal(ctx->global(i))); }
    ...
    if (ctx->array() != null) elp.imports = fromConstants(any_cast<vector<CpInfo>>(visitArray(ctx->array())));
    else
    ...
    for (size_t j = 0; j < elp.constantPoolCount; ++j) { elp.constantPool[j] = constantPool[j]; }
...
std::any AssemblyBaseVisitor::visitGlobal(AssemblyParser::GlobalContext *ctx) {
    ...
    if (type == "VAR") global.flags = 0x01;
        ...
        throw Unreachable();
    global.thisGlobal = fromConstants(any_cast<string>(visitName(ctx->name())));
    global.type = fromConstants(any_cast<Sign>(visitSignature(ctx->signature())).toString());
    return global;
}
...
std::any AssemblyBaseVisitor::visitMethod(AssemblyParser::MethodContext *ctx) {
    ...
    auto &state = getState();
    auto methodName = any_cast<Sign>(visitSignature(ctx->signature())).toString();
    MethodInfo method{};
    if (ctx->kind != null) {
    ...
    method.thisMethod = fromConstants(methodName);

    method.typeParamCount = 0;
    method.typeParams = null;

    method.argsCount = ctx->arg().size();
    method.args = new MethodInfo::ArgInfo[method.argsCount];
    for (size_t i = 0; i < method.argsCount; ++i) { method.args[i] = any_cast<MethodInfo::ArgInfo>(visit(ctx->arg(i))); }

    method.localsCount = ctx->local().size();
    method.closureStart = ctx->NUMBER().size() == 2 ? stoi(ctx->NUMBER(0)->getText()) : 0;
    ...
    method.maxStack = ctx->NUMBER().size() == 2 ? stoi(ctx->NUMBER(1)->getText()) : stoi(ctx->NUMBER(0)->getText());
    ...
    for (size_t i = 0; i < method.exceptionTableCount; ++i) {
        method.exceptionTable[i] = any_cast<MethodInfo::ExceptionTableInfo>(visitExceptionItem(ctx->exceptionItem(i)));
    }
...
std::any AssemblyBaseVisitor::visitArg(AssemblyParser::ArgContext *ctx) {
    ...
    arg.thisArg = fromConstants(any_cast<string>(visitName(ctx->name())));
    arg.type = fromConstants(any_cast<Sign>(visitSignature(ctx->signature())).toString());
    return arg;
}

std::any AssemblyBaseVisitor::visitLocal(AssemblyParser::LocalContext *ctx) {
    MethodInfo::LocalInfo local{};
    local.thisLocal = fromConstants(any_cast<string>(visitName(ctx->name())));
    local.type = fromConstants(any_cast<Sign>(visitSignature(ctx->signature())).toString());
    return local;
}
...
std::any AssemblyBaseVisitor::visitLine(AssemblyParser::LineContext *ctx) {
    ...
    state.mark();
    // Register the label
    if (!label.empty()) { state.addLabel(label); }
    // Append the opcode
    ...
    if (ctx->value() != null) {
        ...
        cpidx value;
        if (OpcodeInfo::takeFromConstPool(opcode)) value = fromConstants(operand);
        else {
            if (operand.tag != 0x04 /* int */) throw AssemblerError("expected integral value");
            value = operand._int;
        }
        switch (params) {
            case 2: {
            ...
            case 1:
                ...
                break;
            default:
                throw AssemblerError(format("opcode expects %d parameters", params), OpcodeInfo::toString(opcode));
        }
...
std::any AssemblyBaseVisitor::visitExceptionItem(AssemblyParser::ExceptionItemContext *ctx) {
    ...
    info.startPc = state.getLabel(ctx->ID(0)->getText());
    info.endPc = state.getLabel(ctx->ID(1)->getText());
    info.targetPc = state.getLabel(ctx->ID(2)->getText());
    info.exception = fromConstants(ctx->ID(0)->getText());
    return info;
}

uint16 AssemblyBaseVisitor::getAccessFlag(const vector<AssemblyParser::AccessorContext *> &accessors) {
    uint16 accessFlag = 0;
    for (auto accessor: accessors) { accessFlag |= any_cast<uint16>(visitAccessor(accessor)); }
    return accessFlag;
}
...
std::any AssemblyBaseVisitor::visitClass(AssemblyParser::ClassContext *ctx) {
    ...
    klass.thisClass = fromConstants(any_cast<Sign>(visitSignature(ctx->signature())).toString());

    klass.typeParamCount = 0;
    klass.typeParams = null;

    klass.supers = ctx->supers != null ? fromConstants(any_cast<vector<CpInfo>>(visitArray(ctx->supers)))
                                       : fromConstants(CpInfo::fromArray({}));

    klass.fieldsCount = ctx->field().size();
    klass.fields = new FieldInfo[klass.fieldsCount];
    for (size_t i = 0; i < klass.fieldsCount; ++i) { klass.fields[i] = any_cast<FieldInfo>(visitField(ctx->field(i))); }

    klass.methodsCount = ctx->method().size();
    klass.methods = new MethodInfo[klass.methodsCount];
    for (size_t i = 0; i < klass.methodsCount; ++i) { klass.methods[i] = any_cast<MethodInfo>(visitMethod(ctx->method(i))); }

    klass.objectsCount = ctx->class_().size();
    klass.objects = new ObjInfo[klass.objectsCount];
    for (size_t i = 0; i < klass.objectsCount; ++i) { klass.objects[i] = any_cast<ObjInfo>(visitClass(ctx->class_(i))); }
...
std::any AssemblyBaseVisitor::visitAccessor(AssemblyParser::AccessorContext *ctx) {
    ...
    const std::map<string, uint16> MODIFIER_TABLE = {
            {"STATIC",          0b0000'0000'0000'0001},
            {"ABSTRACT",        0b0000'0000'0000'0010},
            {"FINAL",           0b0000'0000'0000'0100},
            {"OPERATOR",        0b0000'0000'0000'1000},
            {"PRIVATE",         0b0000'0001'0000'0000},
            {"INTERNAL",        0b0000'0010'0000'0000},
            {"PACKAGE_PRIVATE", 0b0000'0100'0000'0000},
            {"PROTECTED",       0b0000'1000'0000'0000},
            {"PUBLIC",          0b0001'0000'0000'0000},
    };
...
std::any AssemblyBaseVisitor::visitField(AssemblyParser::FieldContext *ctx) {
    ...
    field.thisField = fromConstants(any_cast<string>(visitName(ctx->name())));
    field.type = fromConstants(any_cast<Sign>(visitSignature(ctx->signature())).toString());
    return field;
}

std::any AssemblyBaseVisitor::visitValue(AssemblyParser::ValueContext *ctx) {
    if (ctx->NUMBER() != null) {
        return CpInfo::fromInt(stoi(ctx->NUMBER()->getText()));
    } else if (ctx->STRING() != null) {
        return CpInfo::fromString(removeQuotes(ctx->STRING()->getText()));
    } else if (ctx->CSTRING() != null) {
        return CpInfo::fromChar(ctx->CSTRING()->getText()[1]);
...
std::any AssemblyBaseVisitor::visitArray(AssemblyParser::ArrayContext *ctx) {
    vector<CpInfo> array;
    for (auto value: ctx->value()) { array.push_back(any_cast<CpInfo>(visitValue(value))); }
    return array;
}

std::any AssemblyBaseVisitor::visitFloat(AssemblyParser::FloatContext *ctx) { return stod(ctx->getText()); }

std::any AssemblyBaseVisitor::visitName(AssemblyParser::NameContext *ctx) {
    return ctx->ID() ? ctx->ID()->getText() : removeQuotes(ctx->STRING()->getText());
}

std::any AssemblyBaseVisitor::visitSignature(AssemblyParser::SignatureContext *ctx) {
    return Sign(ctx->getText());
}

std::any AssemblyBaseVisitor::visitSignModule(AssemblyParser::SignModuleContext *ctx) { return std::any(); }

std::any AssemblyBaseVisitor::visitSignClass(AssemblyParser::SignClassContext *ctx) { return std::any(); }

std::any AssemblyBaseVisitor::visitSignMethod(AssemblyParser::SignMethodContext *ctx) { return std::any(); }

std::any AssemblyBaseVisitor::visitSignTypeParams(AssemblyParser::SignTypeParamsContext *ctx) { return std::any(); }

std::any AssemblyBaseVisitor::visitSignParams(AssemblyParser::SignParamsContext *context) { return std::any(); }

std::any AssemblyBaseVisitor::visitSignParam(AssemblyParser::SignParamContext *context) { return std::any(); }
